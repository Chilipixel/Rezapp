...
import { bringService } from './services/bringService.js';
...
function showIngredientPicker(recipe, ingredients) {
  ...
  $('#pick-bring').onclick=async()=>{try{const msg=await bringService.send(chosen());closeModal();toast(msg)}catch(e){if(e.name!=='AbortError')toast(e.message,'error')}};
}
...
function renderShopping() {
  ...
  $('#send-bring').onclick=async()=>{try{toast(await bringService.send(open))}catch(e){if(e.name!=='AbortError')toast(e.message,'error')}};
}
...
function renderSettings() {
  ...
  ['bring-email','bring-password','bring-list'].forEach(id=>$('#'+id).onchange=async e=>{state.settings[{ 'bring-email':'bringEmail','bring-password':'bringPassword','bring-list':'bringList'}[id]]=e.target.value;await save()});
  $('#test-bring').onclick=async()=>{try{await bringService.testConnection({email:$('#bring-email').value,password:$('#bring-password').value})}catch(e){toast(e.message,'error')}};
